...
extern "C" {
    ...
    uint64_t FindNALSize(DecodeAVC *Dec, BitBuffer *BitB) {
        uint64_t NALSize = 0;
        if (Dec != NULL && BitB != NULL) {
            size_t   StartBufferPosition, EndOfBufferPosition;
    ...
    void ParseAVCFile(DecodeAVC *Dec, BitBuffer *BitB) { // byte_stream_nal_unit
        if (Dec != NULL && BitB != NULL) {
            if (ReadBits(MSByteFirst, LSBitFirst, BitB, 32) == AVCMagic && GetBitBufferPosition(BitB) == 0) {
    ...
    void ParseAVCHeader(DecodeAVC *Dec, BitBuffer *BitB) { // nal_unit
        if (Dec != NULL && BitB != NULL) {
            ...
            if (ReadBits(MSByteFirst, MSBitFirst, BitB, 1) == 0) {
                Dec->NAL->NALRefIDC               = ReadBits(MSByteFirst, LSBitFirst, BitB, 2); // 1
                Dec->NAL->NALUnitType             = ReadBits(MSByteFirst, LSBitFirst, BitB, 5); // 7
                ...
                if ((Dec->NAL->NALUnitType == NAL_PrefixUnit) || (Dec->NAL->NALUnitType == NAL_AuxiliarySliceExtension) ||(Dec->NAL->NALUnitType == NAL_MVCDepthView) ) {
                    if (Dec->NAL->NALUnitType == NAL_MVCDepthView) {
                        Dec->NAL->AVC3DExtensionFlag = ReadBits(MSByteFirst, LSBitFirst, BitB, 1);
                    } else {
                        Dec->NAL->SVCExtensionFlag   = ReadBits(MSByteFirst, LSBitFirst, BitB, 1);
                    }
                ...
                for (uint64_t NALByte = BytesInNALUnit; NALByte < Dec->NAL->NALUnitSize; NALByte++) {
                    if (NALByte + 2 < Dec->NAL->NALUnitSize) {
                        uint32_t NALID = ReadBits(MSByteFirst, LSBitFirst, BitB, 24);
                        if (NALID == 0x000003) {
                            uint8_t NALID2 = ReadBits(MSByteFirst, LSBitFirst, BitB, 8);
    ...
    void ExtractNALFromByteStream(DecodeAVC *Dec, BitBuffer *BitB, size_t NALSize) {
        ...
        if (Dec != NULL && BitB != NULL) {
            uint32_t ByteAlignment = ReadBits(MSByteFirst, LSBitFirst, BitB, 32);
            if (ByteAlignment == 0x00000001) { // Bytestream is aligned
                uint32_t StreamType = ReadBits(MSByteFirst, LSBitFirst, BitB, 24);
                ...
                if ((ReadBits(MSByteFirst, LSBitFirst, BitB, 24) == 0x000000) && (IsStreamByteAligned(BitB->BitsUnavailable, 1) == true)) {
    ...
    void ResidualLuma(DecodeAVC *Dec, BitBuffer *BitB, int i16x16DClevel, int i16x16AClevel, int level4x4, int level8x8, int startIdx, int endIdx) { // residual_luma
        if (Dec != NULL && BitB != NULL) {
        ...
        if (Dec != NULL && BitB != NULL) {
            ...
            if (Dec->PPS->EntropyCodingMode == Arithmetic) {
                while (more_rbsp_trailing_data()) {
                    uint64_t CABACZeroWord = ReadBits(MSByteFirst, LSBitFirst, BitB, 16); /* equal to 0x0000 */
                }
            }
        } else if (Dec == NULL) {
            Log(Log_ERROR, __func__, U8("DecodeAVC Pointer is NULL"));
    ...
    uint64_t ScalingList(DecodeAVC *Dec, BitBuffer *BitB, uint8_t *scalingList, size_t ScalingListSize, bool UseDefaultScalingMatrixFlag) { // scaling_list
        ...
        if (Dec != NULL && BitB != NULL) {
            for (uint64_t J = 0; J < ScalingListSize; J++) {
                if (NextScale != 0) {
                    DeltaScale = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, true);
    ...
    void AcquisitionElement3DV(DecodeAVC *Dec, BitBuffer *BitB, uint8_t numViews, uint8_t DeltaFlag, uint8_t PredDirection, uint8_t precMode, uint8_t expLen, uint8_t OutSign[MVCMaxViews], uint8_t OutExp, uint8_t OutMantissa, uint8_t OutManLen[MVCMaxViews]) { // 3dv_acquisition_element
        if (Dec != NULL && BitB != NULL) {
            ...
            if (numViews - DeltaFlag > 1) {
                element_equal_flag = ReadBits(MSByteFirst, LSBitFirst, BitB, 1);
            }
            ...
            for (uint8_t Value = 0; Value < numValues; Value++) {
                if ((PredDirection == 2) && (Value = 0)) {
                    if (precMode == 0) {
                        matissa_len_minus1 = ReadBits(MSByteFirst, LSBitFirst, BitB, 5) + 1;
                        OutManLen[index, Value] = manLen = matissa_len_minus1;
                    } else {
                        prec = ReadBits(MSByteFirst, LSBitFirst, BitB, 5);
                    }
                }
                if (PredDirection == 2) {
                    sign0 = ReadBits(MSByteFirst, LSBitFirst, BitB, 1);
                    OutSign[index, Value] = sign0;
                    exponent0 = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, false);
                    ...
                    mantissa0 = ReadBits(MSByteFirst, LSBitFirst, BitB, manLen);
                    OutMantissa[index, Value] = mantissa0;
                } else {
                    skip_flag = ReadBits(MSByteFirst, LSBitFirst, BitB, 1);
                    if (skip_flag == 0) {
                        sign1 = ReadBits(MSByteFirst, LSBitFirst, BitB, 1);
                        OutSign[index, Value] = sign1;
                        exponent_skip_flag = ReadBits(MSByteFirst, LSBitFirst, BitB, 1);
                        if (exponent_skip_flag == 0) {
                            exponent1 = ReadBits(MSByteFirst, LSBitFirst, BitB, expLen);
                            OutExp[index, Value] = exponent1;
                        } else {
                            OutExp[index, Value] = OutExp[Dec->DPS->ReferenceDPSID[1], Value];
                        }
                        mantissa_diff = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, true);
    ...
    uint64_t DepthRanges(BitBuffer *BitB, uint64_t NumberOfViews, bool PredDirection, uint64_t Index) { // depth_ranges
        if (Dec != NULL && BitB != NULL) {
            bool NearFlag = ReadBits(MSByteFirst, LSBitFirst, BitB, 1);
            bool FarFlag  = ReadBits(MSByteFirst, LSBitFirst, BitB, 1);
        ...
        if (Dec != NULL && BitB != NULL) {
            ...
            for (uint8_t View = 0; View < numViews; View++) { // Param
                for (uint8_t J = 0; J < View; J++) { // Wat
                    disparity_diff_wji[J][View] = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, false);
                    disparity_diff_oji[J][View] = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, false);
                    disparity_diff_wij[View][J] = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, false);
                    disparity_diff_oij[View][J] = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, false);
    ...
    void RefPicListMVCMod(DecodeAVC *Dec, BitBuffer *BitB) { // ref_pic_list_mvc_modification
        if (Dec != NULL && BitB != NULL) {
            if (((Dec->Slice->Type % 5) != 2) && ((Dec->Slice->Type % 5) != 4)) {
                Dec->Slice->RefPicListModFlag[0] = ReadBits(MSByteFirst, LSBitFirst, BitB, 1);
                if (Dec->Slice->RefPicListModFlag[0] == true) {
                    Dec->Slice->ModPicNumsIDC = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, false);
                    if ((Dec->Slice->ModPicNumsIDC == 0) || (Dec->Slice->ModPicNumsIDC == 1)) {
                        Dec->Slice->AbsDiffPicNum = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, true) + 1;
                    } else if (Dec->Slice->ModPicNumsIDC == 2) {
                        Dec->Slice->LongTermPicNum = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, false);
                    } else if ((Dec->Slice->ModPicNumsIDC == 4) || (Dec->Slice->ModPicNumsIDC == 5)) {
                        Dec->Slice->AbsDiffViewIdx = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, false) + 1;
                    }
                }
            }
            if ((Dec->Slice->Type % 5) == 1) {
                Dec->Slice->RefPicListModFlag[1] = ReadBits(MSByteFirst, LSBitFirst, BitB, 1);
    ...
    void pred_weight_table(DecodeAVC *Dec, BitBuffer *BitB) { // pred_weight_table
        if (Dec != NULL && BitB != NULL) {
            Dec->Slice->LumaWeightDenom = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, true);
            if (Dec->SPS->ChromaArrayType != ChromaBW) {
                Dec->Slice->ChromaWeightDenom = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, false);
            }
            for (uint8_t i = 0; i <= Dec->MacroBlock->NumRefIndexActiveLevel0; i++) {
                Dec->Slice->LumaWeightFlag[0] = ReadBits(MSByteFirst, LSBitFirst, BitB, 1);
                if (Dec->Slice->LumaWeightFlag[0] == true) {
                    Dec->Slice->LumaWeight[0][i]  = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, true);
                    Dec->Slice->LumaOffset[0][i]  = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, true);
                }
                if (Dec->SPS->ChromaArrayType != ChromaBW) {
                    Dec->Slice->ChromaWeightFlag[0] = ReadBits(MSByteFirst, LSBitFirst, BitB, 1);
                    if (Dec->Slice->ChromaWeightFlag[0] == true) {
                        for (int J = 0; J < 2; J++) {
                            Dec->Slice->ChromaWeight[0][i][J] = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, true);
                            Dec->Slice->ChromaOffset[0][i][J] = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, true);
                        }
                    }
                }
            }
            if ((Dec->Slice->Type % 5) == 1) {
                for (uint8_t i = 0; i <= Dec->MacroBlock->NumRefIndexActiveLevel1; i++) {
                    Dec->Slice->LumaWeightFlag[1] = ReadBits(MSByteFirst, LSBitFirst, BitB, 1);
                    if (Dec->Slice->LumaWeightFlag[1] == true) {
                        Dec->Slice->LumaWeight[1][i]  = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, true);
                        Dec->Slice->LumaOffset[1][i]  = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, true);
                    }
                    if (Dec->SPS->ChromaArrayType != ChromaBW) {
                        Dec->Slice->ChromaWeightFlag[1] = ReadBits(MSByteFirst, LSBitFirst, BitB, 1);
                        if (Dec->Slice->ChromaWeightFlag[1] == true) {
                            for (uint8_t J = 0; J < 2; J++) {
                                Dec->Slice->ChromaWeight[1][i][J] = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, true);
                                Dec->Slice->ChromaOffset[1][i][J] = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, true);
                            }
                        }
                    }
                }
    ...
    void DecodeRefPicMarking(DecodeAVC *Dec, BitBuffer *BitB) { // dec_ref_pic_marking
        if (Dec != NULL && BitB != NULL) {
            if (Dec->Slice->SliceIsIDR == true) {
                Dec->NAL->EmptyPictureBufferBeforeDisplay = ReadBits(MSByteFirst, LSBitFirst, BitB, 1);
                Dec->NAL->FrameIsLongTermReference        = ReadBits(MSByteFirst, LSBitFirst, BitB, 1);
            } else {
                Dec->NAL->AdaptiveRefPicMarkingModeFlag   = ReadBits(MSByteFirst, LSBitFirst, BitB, 1);
                if (Dec->NAL->AdaptiveRefPicMarkingModeFlag == true) {
                    Dec->NAL->MemManControlOp = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, false);
                    if ((Dec->NAL->MemManControlOp == 1) || (Dec->NAL->MemManControlOp == 3)) {
                        Dec->NAL->PicNumDiff = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, false) + 1;
                    }
                    if (Dec->NAL->MemManControlOp == 2) {
                        Dec->Slice->LongTermPicNum  = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, false);
                    }
                    if ((Dec->NAL->MemManControlOp == 3) || (Dec->NAL->MemManControlOp == 6)) {
                        Dec->NAL->LongTermFrameIndex = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, false);
                    }
                    if (Dec->NAL->MemManControlOp == 4) {
                        Dec->NAL->MaxLongTermFrameIndex = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, false) - 1;
                    }
                }
            }
        } else if (Dec == NULL) {
    ...
    void ParseResidualTransformCoefficentExpGolomb(DecodeAVC *Dec, BitBuffer *BitB, uint64_t CoefficentLevel, uint64_t StartIndex, uint64_t EndIndex, uint64_t MaxCoefficents) { // residual_block_cavlc
        ...
        uint64_t coeff_token = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, false);
            ...
            if (TotalCoeff(coeff_token) > 0) {
                ...
                for (uint64_t Coefficent = 0; Coefficent < TotalCoeff(coeff_token); Coefficent++) {
                    if (Coefficent < TrailingOnes(coeff_token)) {
                        trailing_ones_sign_flag = ReadBits(MSByteFirst, LSBitFirst, BitB, 1);
                        levelVal[Coefficent] = 1 - 2 * trailing_ones_sign_flag;
                    } else {
                        level_prefix = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, false);
                        levelCode = (Min(15, level_prefix) << suffixLength);
                        if ((suffixLength > 0) || (level_prefix >= 14)) {
                            level_suffix = ReadBits(MSByteFirst, LSBitFirst, BitB, 0);
                    ...
                    if (TotalCoeff(coeff_token) < EndIndex - StartIndex + 1) { // FIXME: this may need to be moved
                        total_zeros = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, false);
                    ...
                    for (uint64_t i = 0; i < TotalCoeff(coeff_token) - 1; i++) {
                        if (zerosLeft > 0) {
                            run_before = ReadExpGolomb(MSByteFirst, LSBitFirst, BitB, false);
                            runVal[i] = run_before;
                        } else {
                            runVal[i] = 0;
                        }
        ...
        if (Dec != NULL && BitB != NULL) {
            switch (Dec->NAL->NALUnitType) { // nal_unit_type
                ...
                case NAL_MVCDepthView: // 21
                    ...
                    break;
                default:
                    Log(Log_ERROR,  __func__, U8("NAL ID 0x%X is not supported, seeking to next NAL\n"), Dec->NAL->NALUnitType);
                    break;
            }
        } else if (Dec == NULL) {
            Log(Log_ERROR, __func__, U8("DecodeAVC Pointer is NULL"));
    ...
    void rbsp_trailing_bits(DecodeAVC *Dec, BitBuffer *BitB) { // rbsp_trailing_bits
        if (Dec != NULL && BitB != NULL) {
            ...
            rbsp_stop_one_bit = ReadBits(MSByteFirst, LSBitFirst, BitB, 1);
