...
extern "C" {
    ...
    void ParseSequenceParameterSetData(DecodeAVC *Dec, BitBuffer *BitB) { // seq_parameter_set_data
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSequenceParameterSetData", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSequenceParameterSetData", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseNALSequenceParameterSetExtended(DecodeAVC *Dec, BitBuffer *BitB) { // seq_parameter_set_extension_rbsp?
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSequenceParameterSetExtended", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSequenceParameterSetExtended", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseNALSubsetSPS(DecodeAVC *Dec, BitBuffer *BitB) { // subset_seq_parameter_set_rbsp
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSubsetSPS", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSubsetSPS", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseNALSequenceParameterSet(DecodeAVC *Dec, BitBuffer *BitB) { // seq_parameter_set_rbsp
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSequenceParameterSet", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSequenceParameterSet", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseVideoUsabilityInformation(DecodeAVC *Dec, BitBuffer *BitB) { // Video Usability Information; ParseVUIParameters
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseVideoUsabilityInformation", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseVideoUsabilityInformation", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseMVCDVUIParametersExtension(DecodeAVC *Dec, BitBuffer *BitB) { // mvcd_vui_parameters_extension
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseMVCDVUIParametersExtension", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseMVCDVUIParametersExtension", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseMVCVUIParametersExtension(DecodeAVC *Dec, BitBuffer *BitB) { // mvc_vui_parameters_extension
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseMVCVUIParametersExtension", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseMVCVUIParametersExtension", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSVCVUIExtension(DecodeAVC *Dec, BitBuffer *BitB) { // svc_vui_parameters_extension
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSVCVUIExtension", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSVCVUIExtension", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseNALPictureParameterSet(DecodeAVC *Dec, BitBuffer *BitB) { // pic_parameter_set_rbsp
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALPictureParameterSet", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALPictureParameterSet", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseNALSVCExtension(DecodeAVC *Dec, BitBuffer *BitB) { // nal_unit_header_svc_extension
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSVCExtension", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSVCExtension", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseNALSequenceParameterSetSVC(DecodeAVC *Dec, BitBuffer *BitB) { // seq_parameter_set_svc_extension
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSequenceParameterSetSVC", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSequenceParameterSetSVC", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseNALPrefixUnitSVC(DecodeAVC *Dec, BitBuffer *BitB) { // prefix_nal_unit_svc
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALPrefixUnitSVC", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALPrefixUnitSVC", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseNALMVCExtension(DecodeAVC *Dec, BitBuffer *BitB) { // nal_unit_header_mvc_extension
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALMVCExtension", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALMVCExtension", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSPSMVCDExtension(DecodeAVC *Dec, BitBuffer *BitB) { // seq_parameter_set_mvcd_extension
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSPSMVCDExtension", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSPSMVCDExtension", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseNALDepthParameterSet(DecodeAVC *Dec, BitBuffer *BitB) { // depth_parameter_set_rbsp
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALDepthParameterSet", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALDepthParameterSet", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSPS3DAVCExtension(DecodeAVC *Dec, BitBuffer *BitB) { // seq_parameter_set_3davc_extension
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSPS3DAVCExtension", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSPS3DAVCExtension", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseNAL3DAVCExtension(DecodeAVC *Dec, BitBuffer *BitB) { // nal_unit_header_3davc_extension
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNAL3DAVCExtension", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNAL3DAVCExtension", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseHypotheticalReferenceDecoder(DecodeAVC *Dec, BitBuffer *BitB) { // hrd_parameters
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseHypotheticalReferenceDecoder", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseHypotheticalReferenceDecoder", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseNALSliceHeader(DecodeAVC *Dec, BitBuffer *BitB) { // slice_header
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSliceHeader", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSliceHeader", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseNALSliceData(DecodeAVC *Dec, BitBuffer *BitB, uint8_t Category) { // slice_data
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSliceData", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSliceData", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseNALSlicePartitionA(DecodeAVC *Dec, BitBuffer *BitB) { // slice_data_partition_a_layer_rbsp
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSlicePartitionA", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSlicePartitionA", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseNALSlicePartitionB(DecodeAVC *Dec, BitBuffer *BitB) { // slice_data_partition_b_layer_rbsp
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSlicePartitionB", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSlicePartitionB", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseNALSlicePartitionC(DecodeAVC *Dec, BitBuffer *BitB) { // slice_data_partition_c_layer_rbsp
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSlicePartitionC", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSlicePartitionC", "Pointer to BitBuffer is NULL");
        } else {
            uint8_t SliceID                                                             = ReadExpGolomb(BitIOMSByte, BitIOLSBit, BitB, false);
            if (Dec->SPS->SeperateColorPlane == true) {
                uint8_t ColorPlaneID                                                    = ReadExpGolomb(BitIOMSByte, BitIOLSBit, BitB, false);
    ...
    void ParseNALSliceNonPartitioned(DecodeAVC *Dec, BitBuffer *BitB) { // slice_layer_without_partitioning_rbsp
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSliceNonPartitioned", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALSliceNonPartitioned", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseNALFillerData(DecodeAVC *Dec, BitBuffer *BitB) { // filler_data_rbsp
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALFillerData", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALFillerData", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseNALPrefixUnit(DecodeAVC *Dec, BitBuffer *BitB) { // prefix_nal_unit_rbsp
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALPrefixUnit", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALPrefixUnit", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseNALAccessUnitDelimiter(DecodeAVC *Dec, BitBuffer *BitB) { // access_unit_delimiter_rbsp
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALAccessUnitDelimiter", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseNALAccessUnitDelimiter", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIBufferingPeriod(DecodeAVC *Dec, BitBuffer *BitB) { // buffering_period
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIBufferingPeriod", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIBufferingPeriod", "Pointer to BitBuffer is NULL");
        } else {
            Dec->SPS->SeqParamSetID                                                     = ReadExpGolomb(BitIOMSByte, BitIOLSBit, BitB, false);
    ...
    void ParseSEIPictureTiming(DecodeAVC *Dec, BitBuffer *BitB) { // pic_timing
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIPictureTiming", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIPictureTiming", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIPanScan(DecodeAVC *Dec, BitBuffer *BitB) { // pan_scan_rect
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIPanScan", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIPanScan", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIFiller(DecodeAVC *Dec, BitBuffer *BitB) { // filler_payload
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIFiller", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIFiller", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIRegisteredUserData(DecodeAVC *Dec, BitBuffer *BitB) { // user_data_registered_itu_t_t35
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIRegisteredUserData", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIRegisteredUserData", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIUnregisteredUserData(DecodeAVC *Dec, BitBuffer *BitB) { // user_data_unregistered
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIUnregisteredUserData", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIUnregisteredUserData", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIRecoveryPoint(DecodeAVC *Dec, BitBuffer *BitB) { // recovery_point
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIRecoveryPoint", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIRecoveryPoint", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIRepetitiveReferencePicture(DecodeAVC *Dec, BitBuffer *BitB) { // dec_ref_pic_marking_repetition
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIRepetitiveReferencePicture", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIRepetitiveReferencePicture", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEISparePicture(DecodeAVC *Dec, BitBuffer *BitB) { // spare_pic
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEISparePicture", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEISparePicture", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEISceneInfo(DecodeAVC *Dec, BitBuffer *BitB) { // scene_info
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEISceneInfo", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEISceneInfo", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEISubSequenceInfo(DecodeAVC *Dec, BitBuffer *BitB) { // sub_seq_info
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEISubSequenceInfo", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEISubSequenceInfo", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEISubSequenceLayerProperties(DecodeAVC *Dec, BitBuffer *BitB) { // sub_seq_layer_characteristics
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEISubSequenceLayerProperties", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEISubSequenceLayerProperties", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEISubSequenceProperties(DecodeAVC *Dec, BitBuffer *BitB) { // sub_seq_characteristics
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEISubSequenceProperties", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEISubSequenceProperties", "Pointer to BitBuffer is NULL");
        } else {
            Dec->SEI->SubSequenceLayerNum                                                = ReadExpGolomb(BitIOMSByte, BitIOLSBit, BitB, false);
            Dec->SEI->SubSequenceID                                                      = ReadExpGolomb(BitIOMSByte, BitIOLSBit, BitB, false);
    ...
    void ParseSEIFullFrameFreeze(DecodeAVC *Dec, BitBuffer *BitB) { // full_frame_freeze
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIFullFrameFreeze", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIFullFrameFreeze", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIFullFrameFreezeRelease(DecodeAVC *Dec, BitBuffer *BitB) { // full_frame_freeze_release
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIFullFrameFreezeRelease", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIFullFrameFreezeRelease", "Pointer to BitBuffer is NULL");
        } else {
            SkipBits(BitB, Bytes2Bits(Dec->SEI->SEISize));
        }
    }
    ...
    void ParseSEIFullFrameSnapshot(DecodeAVC *Dec, BitBuffer *BitB) { // full_frame_snapshot
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIFullFrameSnapshot", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIFullFrameSnapshot", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIProgressiveRefinementSegmentStart(DecodeAVC *Dec, BitBuffer *BitB) { // progressive_refinement_segment_start
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIProgressiveRefinementSegmentStart", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIProgressiveRefinementSegmentStart", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIProgressiveRefinementSegmentEnd(DecodeAVC *Dec, BitBuffer *BitB) { // progressive_refinement_segment_end
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIProgressiveRefinementSegmentEnd", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIProgressiveRefinementSegmentEnd", "Pointer to BitBuffer is NULL");
        } else {
            Dec->SEI->ProgressiveRefinementID                                            = ReadExpGolomb(BitIOMSByte, BitIOLSBit, BitB, false);
        }
    }
    ...
    void ParseSEIMotionConstrainedSliceGroupSet(DecodeAVC *Dec, BitBuffer *BitB) { // motion_constrained_slice_group_set
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIMotionConstrainedSliceGroupSet", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIMotionConstrainedSliceGroupSet", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIFilmGrainCharacteristics(DecodeAVC *Dec, BitBuffer *BitB) { // film_grain_characteristics
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIFilmGrainCharacteristics", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIFilmGrainCharacteristics", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIDeblockingFilterDisplayPreference(DecodeAVC *Dec, BitBuffer *BitB) { // deblocking_filter_display_preference
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIDeblockingFilterDisplayPreference", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIDeblockingFilterDisplayPreference", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIStereoVideoInfo(DecodeAVC *Dec, BitBuffer *BitB) { // stereo_video_info
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIStereoVideoInfo", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIStereoVideoInfo", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIPostFilterHint(DecodeAVC *Dec, BitBuffer *BitB) { // post_filter_hint
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIPostFilterHint", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIPostFilterHint", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIToneMappingInfo(DecodeAVC *Dec, BitBuffer *BitB) { // tone_mapping_info
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIToneMappingInfo", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIToneMappingInfo", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIScalabilityInfo(DecodeAVC *Dec, BitBuffer *BitB) { // scalability_info
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIScalabilityInfo", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIScalabilityInfo", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEISubPictureScalableLayer(DecodeAVC *Dec, BitBuffer *BitB) { // sub_pic_scalable_layer
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEISubPictureScalableLayer", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEISubPictureScalableLayer", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEINonRequiredLayerRep(DecodeAVC *Dec, BitBuffer *BitB) { // non_required_layer_rep
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEINonRequiredLayerRep", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEINonRequiredLayerRep", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIPriorityLayerInfo(DecodeAVC *Dec, BitBuffer *BitB) { // priority_layer_info
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIPriorityLayerInfo", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIPriorityLayerInfo", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEILayersNotPresent(DecodeAVC *Dec, BitBuffer *BitB) { // layers_not_present
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEILayersNotPresent", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEILayersNotPresent", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEILayerDependencyChange(DecodeAVC *Dec, BitBuffer *BitB) { // layer_dependency_change
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEILayerDependencyChange", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEILayerDependencyChange", "Pointer to BitBuffer is NULL");
        } else {
            Dec->SEI->NumLayers = ReadExpGolomb(BitIOMSByte, BitIOLSBit, BitB, false) + 1;
            for (uint8_t Layer = 0; Layer < Dec->SEI->NumLayers; Layer++) {
                Dec->SEI->LayerID[Layer] = ReadExpGolomb(BitIOMSByte, BitIOLSBit, BitB, false);
    ...
    void ParseSEIScalableNesting(DecodeAVC *Dec, BitBuffer *BitB) { // scalable_nesting
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIScalableNesting", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIScalableNesting", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIBaseLayerTemporalHRD(DecodeAVC *Dec, BitBuffer *BitB) { // base_layer_temporal_hrd
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIBaseLayerTemporalHRD", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIBaseLayerTemporalHRD", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIQualityLayerIntegrityCheck(DecodeAVC *Dec, BitBuffer *BitB) { // quality_layer_integrity_check
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIQualityLayerIntegrityCheck", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIQualityLayerIntegrityCheck", "Pointer to BitBuffer is NULL");
        } else {
            Dec->SEI->NumInfoEntries = ReadExpGolomb(BitIOMSByte, BitIOLSBit, BitB, false) + 1;
    ...
    void ParseSEIRedundantPicProperty(DecodeAVC *Dec, BitBuffer *BitB) { // redundant_pic_property
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIRedundantPicProperty", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIRedundantPicProperty", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEITemporalDependencyRepresentationIndex(DecodeAVC *Dec, BitBuffer *BitB) { // tl0_dep_rep_index
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEITemporalDependencyRepresentationIndex", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEITemporalDependencyRepresentationIndex", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEITemporalLevelSwitchingPoint(DecodeAVC *Dec, BitBuffer *BitB) { // tl_switching_point
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEITemporalLevelSwitchingPoint", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEITemporalLevelSwitchingPoint", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIParallelDecodingInfo(DecodeAVC *Dec, BitBuffer *BitB) { // parallel_decoding_info
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIParallelDecodingInfo", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIParallelDecodingInfo", "Pointer to BitBuffer is NULL");
        } else {
            Dec->SPS->SeqParamSetID = ReadExpGolomb(BitIOMSByte, BitIOLSBit, BitB, false);
            for (uint8_t View = 0; View <= Dec->SPS->ViewCount; View++) {
    ...
    void ParseSEIMVCScalableNesting(DecodeAVC *Dec, BitBuffer *BitB) { // mvc_scalable_nesting
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIMVCScalableNesting", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIMVCScalableNesting", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIViewScalabilityInfo(DecodeAVC *Dec, BitBuffer *BitB) { // view_scalability_info FIXME: FINISH THIS FUNCTION!!!
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIViewScalabilityInfo", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIViewScalabilityInfo", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIMVCSceneInfo(DecodeAVC *Dec, BitBuffer *BitB) { // multiview_scene_info
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIMVCSceneInfo", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIMVCSceneInfo", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIMVCAcquisitionInfo(DecodeAVC *Dec, BitBuffer *BitB) { // multiview_acquisition_info
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIMVCAcquisitionInfo", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIMVCAcquisitionInfo", "Pointer to BitBuffer is NULL");
        } else {
            Dec->SPS->ViewCount            = ReadExpGolomb(BitIOMSByte, BitIOLSBit, BitB, false) + 1;
    ...
    void ParseSEINonRequiredViewComponent(DecodeAVC *Dec, BitBuffer *BitB) { // non_required_view_component
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEINonRequiredViewComponent", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEINonRequiredViewComponent", "Pointer to BitBuffer is NULL");
        } else {
            Dec->SEI->NumInfoEntries                               = ReadExpGolomb(BitIOMSByte, BitIOLSBit, BitB, false) + 1;
    ...
    void ParseSEIViewDependencyChange(DecodeAVC *Dec, BitBuffer *BitB) { // view_dependency_change
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIViewDependencyChange", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIViewDependencyChange", "Pointer to BitBuffer is NULL");
        } else {
            Dec->SPS->SeqParamSetID       = ReadExpGolomb(BitIOMSByte, BitIOLSBit, BitB, false);
    ...
    void ParseSEIOperationPointNotPresent(DecodeAVC *Dec, BitBuffer *BitB) { // operation_point(s)_not_present
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIOperationPointNotPresent", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIOperationPointNotPresent", "Pointer to BitBuffer is NULL");
        } else {
            Dec->SEI->NumOperationPoints = ReadExpGolomb(BitIOMSByte, BitIOLSBit, BitB, false) + 1;
            for (uint8_t OperationPoint = 0; OperationPoint < Dec->SEI->NumOperationPoints; OperationPoint++) {
    ...
    void ParseSEIBaseViewTemporalHRD(DecodeAVC *Dec, BitBuffer *BitB) { // base_view_temporal_hrd
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIBaseViewTemporalHRD", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIBaseViewTemporalHRD", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIFramePackingArrangement(DecodeAVC *Dec, BitBuffer *BitB) { // frame_packing_arrangement
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIFramePackingArrangement", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIFramePackingArrangement", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIMVCViewPosition(DecodeAVC *Dec, BitBuffer *BitB) { // multiview_view_position
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIMVCViewPosition", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIMVCViewPosition", "Pointer to BitBuffer is NULL");
        } else {
            Dec->SPS->ViewCount = ReadExpGolomb(BitIOMSByte, BitIOLSBit, BitB, false) + 1;
            for (uint8_t View = 0; View < Dec->SPS->ViewCount; View++) {
    ...
    void ParseSEIDisplayOrientation(DecodeAVC *Dec, BitBuffer *BitB) { // display_orientation
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIDisplayOrientation", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIDisplayOrientation", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIDepthRepresentationInformation(DecodeAVC *Dec, BitBuffer *BitB) { // depth_representation_info
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIDepthRepresentationInformation", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIDepthRepresentationInformation", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEI3DReferenceDisplayInfo(DecodeAVC *Dec, BitBuffer *BitB) { // three_dimensional_reference_displays_info
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEI3DReferenceDisplayInfo", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEI3DReferenceDisplayInfo", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIDepthTiming(DecodeAVC *Dec, BitBuffer *BitB) { // depth_timing
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIDepthTiming", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIDepthTiming", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIDepthGridPosition(DecodeAVC *Dec, BitBuffer *BitB) { // depth_grid_position()
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIDepthGridPosition", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIDepthGridPosition", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIDepthSamplingInfo(DecodeAVC *Dec, BitBuffer *BitB) { // depth_sampling_info
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIDepthSamplingInfo", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIDepthSamplingInfo", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIConstrainedDepthParameterSetID(DecodeAVC *Dec, BitBuffer *BitB) { // constrained_depth_parameter_set_identifier
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIConstrainedDepthParameterSetID", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIConstrainedDepthParameterSetID", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIMeteringDisplayColorVolume(DecodeAVC *Dec, BitBuffer *BitB) { // mastering_display_color_volume
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIMeteringDisplayColorVolume", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIMeteringDisplayColorVolume", "Pointer to BitBuffer is NULL");
        } else {
            for (uint8_t Color = 0; Color < 3; Color++) {
    ...
    void ParseSEIMVCDScalableNesting(DecodeAVC *Dec, BitBuffer *BitB) { // mvcd_scalable_nesting
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIMVCDScalableNesting", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIMVCDScalableNesting", "Pointer to BitBuffer is NULL");
        } else {
            Dec->SEI->OperationPointFlag                                  = ReadBits(BitIOMSByte, BitIOLSBit, BitB, 1);
            if (Dec->SEI->OperationPointFlag == false) {
                Dec->SEI->AllViewComponentsInAUFlag                       = ReadBits(BitIOMSByte, BitIOLSBit, BitB, 1);
    ...
    void ParseSEIDepthRepresentationElement(BitBuffer *BitB, uint8_t OutSign, uint8_t OutExp, uint8_t OutMantissa, uint8_t OutManLen) { // depth_representation_sei_element
        if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIDepthRepresentationElement", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void SkipSEIReservedMessage(BitBuffer *BitB, size_t SEISize) { // reserved_sei_message
        if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "SkipSEIReservedMessage", "Pointer to BitBuffer is NULL");
        } else {
    ...
    void ParseSEIMessage(DecodeAVC *Dec, BitBuffer *BitB) { // sei_message
        if (Dec == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIMessage", "Pointer to DecodeAVC is NULL");
        } else if (BitB == NULL) {
            BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIMessage", "Pointer to BitBuffer is NULL");
        } else {
            while (PeekBits(BitIOMSByte, BitIOLSBit, BitB, 8) == 0xFF) {
                SkipBits(BitB, 8);
                Dec->SEI->SEIType += 255;
            ...
            switch (Dec->SEI->SEIType) { // sei_payload
                ...
                case SEI_MasteringDisplayColorVolume:             // 137
                    ...
                    break;
                default:
                    BitIOLog(LOG_ERROR, "libModernAVC", "ParseSEIMessage", "Unrecognized SEIType: %d", Dec->SEI->SEIType);
